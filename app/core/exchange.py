...
# 복붙 길에서 한 번에 보낼 줄 수.
#
# **300 이었다.** 트랙 하나가 보통 120줄이라 「묶어서 복사」 가 두 트랙에서
# 걸렸다 — 다섯 트랙을 고르고 눌러도 둘만 담기고, 나머지는 답을 넣기 전에는
# 아무리 다시 눌러도 안 나왔다. 다섯 트랙이면 복사·붙여넣기를 세 번 왕복해야
# 해서 「묶어서」 의 뜻이 없었다.
#
# 지금 쓰는 AI 들(ChatGPT · Claude · Gemini · Grok)은 이 정도를 한 번에
# 받는다. 줄이 빠지면 앱이 줄 수를 세어 짚어 주므로, 크게 보내다 흘리는 것은
# 조용히 넘어가지 않는다. 적게 보내고 싶으면 설정에서 내릴 수 있다.
BATCH_LINES = 600

# 손으로 내릴 수 있는 폭. 여기가 상한이라 기본값이 천장에 붙지 않는다
묶음최대 = 1000


def 묶이는자리(줄수들: list[int], 한도: int) -> list[int]:
    """여러 트랙을 한 프롬프트로 묶을 때 **몇 번째가 실제로 담기나.**

    위에서부터 담되 한도를 넘으면 건너뛴다. **첫 트랙은 한도를 넘어도 담는다**
    — 안 그러면 긴 트랙 하나가 아무것도 못 보내는 자리가 된다.

    이 규칙이 두 곳에 있으면 단추가 「5트랙」 이라 써 놓고 창구가 둘만 담는
    일이 다시 난다. 창구(`api._묶을것고르기`)와 화면(`app.js 묶이는수`)이
    이것을 따르고, 하네스 9) 가 둘이 같은 답을 내는지 잰다.
    """
    난것: list[int] = []
    줄수 = 0
    for 자리, 길이 in enumerate(줄수들):
        if 길이 <= 0:
            continue
        if 난것 and 줄수 + 길이 > 한도:
            continue
