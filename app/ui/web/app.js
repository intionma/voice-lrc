...
function 작업그리기() {
  ...
  if (전부묶기) {
    ...
    if (!전부묶기.hidden) {
      const 자리들 = 번역할것.map((j) => j.index);
      전부묶기.textContent =
        `전부 묶어서 복사 (${걸친작품}작품 · ${묶음말(자리들)})`;
      전부묶기.title = "작품을 넘나들어 한 프롬프트로 묶습니다."
        + " 답은 번호를 보고 제 트랙으로 갈라집니다." + 묶음설명(자리들);
      전부묶기.onclick = 안전하게(
...
function 선택바그리기() {
  ...
  if (번역할것.length >= 2) {
    ...
    묶기.type = "button";
    묶기.className = "sb p";
    const 자리들 = 번역할것.map((x) => x.job.index);
    묶기.textContent = `번역 묶어서 복사 (${묶음말(자리들)})`;
    묶기.title = "고른 트랙을 한 프롬프트로 묶습니다." + 묶음설명(자리들);
    묶기.onclick = 안전하게(() => 묶어서복사(자리들, 묶기), "묶지 못했습니다");
    바.appendChild(묶기);
  }
...
// 이만큼 고르면 **몇 트랙이 실제로 담기나.**
//
// 단추가 「묶어서 복사 (5트랙)」 이라 써 놓고 창구는 한도에 걸려 둘만 담았다.
// 못 담은 셋은 답을 넣기 전에는 아무리 다시 눌러도 안 나와서, 같은 것을
// 몇 번씩 복사하게 됐다.
//
// **창구의 `_묶을것고르기` 와 같은 규칙이다** — 위에서부터 담되, 한도를 넘으면
// 건너뛰고, 첫 트랙은 한도를 넘어도 담는다. 둘이 어긋나지 않는지는
// `press_all_buttons.py` 의 9) 가 잰다.
function 묶이는수(자리들) {
  const 한도 = 마지막상태.묶음한도 || 0;
  const 줄수맵 = new Map();
  const 열쇠맵 = new Map();
  for (const j of (마지막상태.jobs || [])) {
    줄수맵.set(j.index, j.next_lines || 0);
    열쇠맵.set(j.index, j.group_key || "");
  }
  const 담은열쇠 = new Set();
  let 줄수 = 0;
  for (const 자리 of 자리들) {
    const 길이 = 줄수맵.get(자리) || 0;
    if (!길이) continue;
    const 열쇠 = 열쇠맵.get(자리) || "";
    if (담은열쇠.has(열쇠)) continue;     // 한 트랙을 두 번 담지 않는다
    if (담은열쇠.size && 줄수 + 길이 > 한도) continue;
    담은열쇠.add(열쇠);
    줄수 += 길이;
  }
  return 담은열쇠.size;
...
/** 「5트랙」 이 아니라 「5트랙 중 2」. 다 담기면 그냥 「5트랙」 */
function 묶음말(자리들) {
  const 담김 = 묶이는수(자리들);
  const 전체 = 자리들.length;
  return 담김 < 전체 ? `${전체}트랙 중 ${담김}` : `${전체}트랙`;
}

function 묶음설명(자리들) {
  const 담김 = 묶이는수(자리들);
  if (담김 >= 자리들.length) return "";
  return ` 한 번에 보낼 줄 수(${마지막상태.묶음한도 || 0}줄)를 넘어서`
    + ` ${자리들.length - 담김}개는 이번에 안 갑니다.`
    + " 답을 넣으면 다음 것이 나옵니다. 설정 › 번역에서 줄 수를 올릴 수 있습니다.";
}
...
async function 묶어서복사(자리들, 단추) {
  ...
  알림(
    ...
    // **왜 남았는지와 무엇을 하면 되는지까지 적는다.** 「다음에」 만 적어
    // 두면 다시 눌러 보게 되는데, 답을 넣기 전에는 같은 것만 나온다
    + (미룬것.length
       ? ` 남은 ${미룬것.length}개(${미룬것.join(" · ")})는 한 번에 보낼 줄 수`
         + `(${마지막상태.묶음한도 || 0}줄)를 넘어서 못 담았습니다.`
         + " 이 답을 넣으면 이어서 나옵니다 — 설정 › 번역에서 줄 수를 올려도 됩니다."
       : ""),
    미룬것.length ? "bad" : "ok");
  await 번역칸그리기();
}
...
function 작품머리(작품) {
  ...
  if (이작품번역할것.length >= 2) {
    ...
    const 자리들 = 이작품번역할것.map((j) => j.index);
    묶기.textContent = `묶어서 복사 (${묶음말(자리들)})`;
